...
// using simd, data aligning, openmp ,fma and cache blocking
#include <stdio.h>
#include <string.h>
#include <iostream>
#include <fstream>
...
const int count = 80;
const int block = 400;
const int local_num = block/count;
...
for(int j=0;j<m;j+=block){
 #pragma omp for 
    ...
    {
      	double result;    
        result = logDataVSPrior(dat_0, dat_1,pri_0, pri_1,ctf, sigRcp, j, disturb[t]);
        temp_data[t]+=result;
        //fout << t+1 << ": " << result <<"\n";
    }
}
...
{
   // double result = 0.0;
   // #pragma omp parallel num_threads(count) reduction(+:result)
