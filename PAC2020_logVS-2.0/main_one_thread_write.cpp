...
//using  one specific thread to wirte to the file.
#include <stdio.h>
#include <string.h>
#include <iostream>
#include <fstream>
...
const int out_loop = 96;
int count=0;



inline void logDataVSPrior(const double* dat_0, const double* dat_1,const double* pri_0, const double* pri_1, const double* ctf, const double* sigRcp, const int num, const double disturb0,const int t,double* temp_data);
...
{ 
    ...
    #pragma omp parallel for num_threads(out_loop)
    for(unsigned int t = 0; t < K; t++)
    {      
       logDataVSPrior(dat_0, dat_1,pri_0, pri_1,ctf, sigRcp, m, disturb[t],t,temp_data);
...
inline void logDataVSPrior(const double* dat_0, const double* dat_1,const double* pri_0, const double* pri_1, const double* ctf, const double* sigRcp, const int num, const double disturb0,const int t,double* temp_data)
{
    ...
    {   
        ...
        m1 = _mm256_fmadd_pd(m_pri0,m_ctf,m_dat0);
        m2 = _mm256_fmadd_pd(m_pri1,m_ctf,m_dat1);
        m3 = _mm256_mul_pd(m1,m1);
        // m4 = _mm256_mul_pd(m2,m2);
        // m3 = _mm256_add_pd(m3,m4);
        m4 = _mm256_fmadd_pd(m2,m2,m3);
        //m5 = _mm256_mul_pd(m4,m_sigRcp);
        local_my_result = _mm256_fmadd_pd(m4,m_sigRcp,local_my_result);
    ...
    result=(my_result_array[0]+my_result_array[1])+(my_result_array[2]+my_result_array[3]);
    ...
    if(my_rank==0&&t==1041){
        ...
        fout.open("result.dat");
        while(count<K){
            while(temp_data[count]==0);
            
            fout << count+1s << ": " << temp_data[count++] <<"\n";
